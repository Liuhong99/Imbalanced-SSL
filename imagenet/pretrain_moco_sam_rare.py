...
import moco.loader
import moco.builder
from dataset.imagenet1 import ImageNetLT
...
parser.add_argument('--root_path', type=str, default='./data2')
...
parser.add_argument('--phi', default=1.003, type=float,
                    help='phi',
                    dest='phi')
...
def CrossEntropyLoss1(label, predict_prob, class_level_weight = None, instance_level_weight = None, epsilon = 1e-12):

    predict_prob = nn.Softmax(dim=1)(predict_prob)
    label = F.one_hot(label, predict_prob.size(1))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or \
            ...
            if (epoch + 1) % args.print_freq == 0:
                ...
                }, is_best=False, filename=f'{args.root_model}/moco_ckpt_{(epoch+1):04d}_sam_rare.pth.tar')
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, gt) in enumerate(train_loader):
        ...
        weight1 = (args.phi ** gt) .float()
        # compute output
        output, target = model(im_q=images[0], im_k=images[1])
        
        #print(output.shape, target.shape)
        loss = CrossEntropyLoss1(predict_prob = output, label = target, instance_level_weight = weight1)
