...
def main(args: argparse.Namespace):
    ...
    if args.pretrained:
        if os.path.isfile(args.pretrained):
            ...
            print(f"TOTAL: {len(list(backbone.state_dict().keys()))}\n======")
            weights = {k: weights[k] for k in weights if k in backbone.state_dict()}
            print(f"Pretrained weights found (TOTAL: {len(list(weights.keys()))}):\n{weights.keys()}\n")
            x = backbone.state_dict()
            x.update(weights)
            backbone.load_state_dict(x)
