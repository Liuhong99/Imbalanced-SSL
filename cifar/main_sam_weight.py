...
from simsiam.criterion import SimSiamLossRe
from simsiam.validation import KNNValidation
from imbalance_cifar import IMBALANCECIFARIND
from sam import SAM
...
parser.add_argument('--weight_path', type=str, help='path to kde weight')
...
def main():
    ...
    train_set = IMBALANCECIFARIND(root=args.data_root,
                                 train=True,
                                 download=True,
                                 transform=TwoCropsTransform(train_transforms), imb_type=args.type, weight_path = args.weight_path)
    ...
    criterion = SimSiamLossRe(args.loss_version)
    ...
    for epoch in range(start_epoch, args.epochs+1):
        ...
        if epoch % args.eval_freq == 0:
            ...
            if val_top1_acc > best_acc:
                ...
                save_checkpoint(epoch, model, optimizer, best_acc,
                                path.join(trial_dir, 'sam{}_best.pth'.format(args.trial)),
        ...
        if epoch % args.save_freq == 0:
            save_checkpoint(epoch, model, optimizer, val_top1_acc,
                            path.join(trial_dir, 'samckpt_epoch_{}_{}.pth'.format(epoch, args.trial)),
                            'Saving...')
    ...
    # save model
    save_checkpoint(epoch, model, optimizer, val_top1_acc,
                    path.join(trial_dir, 'sam{}_last.pth'.format(args.trial)),
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, targets, weights) in enumerate(train_loader):
        ...
        weight1 = weights.float()
        weight2 = torch.ones_like(targets)
        ...
        #optimizer.zero_grad()
        criterion(outs['z1'], outs['z2'], outs['p1'], outs['p2'], weight1).backward()
        ...
        outs = model(im_aug1=images[0], im_aug2=images[1])
        loss = criterion(outs['z1'], outs['z2'], outs['p1'], outs['p2'], weight2)
        loss.backward()
